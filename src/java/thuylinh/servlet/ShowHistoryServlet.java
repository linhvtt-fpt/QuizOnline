...
public class ShowHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<QuizDetailDTO> history = quizDetailDAO.historyQuiz(email, subject);
            if (history != null) {
                request.setAttribute("HISTORY", history);
                url = "showHistory_PAGE";
                url = index.get(url);
            }

        } catch (SQLException e) {
            log("ShowHistoryServlet SQLException " + e.getMessage());
        } catch (NamingException e) {
            log("ShowHistoryServlet NamingException " + e.getMessage());
        } finally {
